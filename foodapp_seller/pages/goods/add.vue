        ...
        <view class="detail-list" v-if="goodsDetailList.length > 0">
          ...
          >
            ...
            <view class="detail-actions">
              <button class="detail-action-btn edit-btn" @click="editDetail(index)">
                <uni-icons type="compose" size="18" color="#fff"></uni-icons>
                编辑
              </button>
              <button class="detail-action-btn delete-btn" @click="deleteDetail(index)">
...
export default {
  ...
  methods: {
      ...
      this.$refs.form.validate().then(async () => {
        ...
        try {
          ...
          if (this.goodsDetailList.length > 0) {
            totalStock = this.goodsDetailList.reduce((sum, detail) => sum + (detail.stockNum || 0), 0)
          }
          
          // 设置默认值
          const defaultRestBuyNum = this.form.restBuyNum || 0
          
          const formData = {
            ...this.form,
            sid: sid,
            stockTotalNum: totalStock,
            restBuyNum: defaultRestBuyNum
          }
          
          // 调用接口提交商品主表数据
          const res = await request.post('/seller/goods/add', formData)
          
          // 获取新添加的商品ID
          const newGoodsId = res.data?.gid
          if (newGoodsId && this.goodsDetailList.length > 0) {
            // 批量添加商品详情
            for (const detail of this.goodsDetailList) {
              detail.gid = newGoodsId
              await request.post('/seller/goods/detail/add', detail)
            }
          }
    ...
    editDetail(index) {
      ...
      const detail = { ...this.goodsDetailList[index] }
      detail.images = detail.gdpic ? detail.gdpic.split(';').map(url => ({ url: url.trim() })) : []
    ...
    submitDetailForm() {
      if (this.isEditDetail) {
        // 更新商品详情
        this.goodsDetailList[this.editIndex] = { ...this.detailForm }
        uni.showToast({
          title: '商品详情更新成功',
          icon: 'success'
        })
        ...
        // 添加商品详情
        this.goodsDetailList.push({ ...this.detailForm })
        uni.showToast({
          title: '商品详情添加成功',
          icon: 'success'
        })
